...
[Integrant](#clojure-integrant) をベースにした拡張機能を提供(作者が同じ).
...
Integrantの考え方としてnamespaceにatomをbindingしない.
その代わりに init-keyの中で atomを宣言して返り値のmapにbindingする.
...
timbreはpure clojureではあるが,
やはり設定例やノウハウがGoogle検索で見つかるのは,
Javaライブラリが多い(logback).
...
また現実的な問題点として,
timbreはpure clojureではあるものの,
3rd party libraryがjavaのライブラリに依存していると,
その制御をtimbreからはできない(Javaから独立していることが利点なので).
そのため結局 logback.xmlを書かないといけないかもしれない.

<https://github.com/ptaoussanis/timbre/issues/138>


### 💡 tips: Jetty サーバのログを黙らせる {#tips-jetty-サーバのログを黙らせる}
