...
### Design Journal Tisp with Clojure Cider {#d2b9a2}
...
アプリをシングルトンなJVMとして起動するのではなくてインスタンスとして起動することで古いインスタンスは捨ててしまう(i.e.ガーベージコレクションかな?). とくにresetを連発してライトにシステム再起動, CIDERで M-x cider-ns-refreshの前後にsuspendとresumeをhookさせるとよいとか.
