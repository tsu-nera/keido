...
との連携が{:as :json} でできる.
この指定がされると clj-httpはrepsonse dataをcheshire でjsonにパースする.

なお 単純に {:headers {:accept "application/json"}}を設定したいだけなら
{:accept :json}を指定する.

ref: [💡Accept と Content-Typeの違い]({{< relref "20220324072101.md#http-header" >}})
