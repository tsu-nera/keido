...
ref: [アリティ]({{< relref "20220305080412.md#アリティ-arity" >}}) [オーバーロード]({{< relref "20220310125619.md#overload-オーバーロード-多重定義" >}})


### Clojure: 可変長引数関数 | Variadic Functions {#clojure-可変長引数関数-variadic-functions}

[可変長引数]({{< relref "20220305075933.md#可変長引数--オプション引数--variadic-variable" >}}) をサポートする関数, variadic = 可変長, aka. オプション引数.
...
core.matchの形式で条件を [Multi-arity Functions](#clojure-multi-arity-functions) の形式で処理を記述することができる.
...
同じことを Clojureでやろうとするならば [cond-> 条件つきスレッドマクロ]({{< relref "20220116162321.md#cond-条件つきスレッドマクロ" >}}) の機能がそれに該当するかもしれないが,
...
[🏷無名関数]({{< relref "20220305080412.md#無名関数--annonimous-functions" >}}) [🏷クロージャ]({{< relref "20220305080412.md#closure-クロージャ" >}})
...
ref: [部分適用]({{< relref "20220305080412.md#部分適用" >}})
...
[💡カリー化と部分適用の違い]({{< relref "20220305080412.md#カリー化と部分適用の違い" >}}) については別ページでまとめたので要確認.
...
### 一時変数(let)を分解するために分配束縛をつかう {#一時変数--let--を分解するために分配束縛をつかう}
...
-   refs.
    -   [Clojureの繰り返し: map vs doseq]({{< relref "20220116083656.md#clojureの繰り返し-map-vs-doseq--for" >}})
    -   [Lazy Evaluation: 遅延評価]({{< relref "20220305075933.md#lazy-evaluation-遅延評価" >}})


### 💡そもそもなぜClojureは設計思想として遅延評価なのか？ {#そもそもなぜclojureは設計思想として遅延評価なのか}
...
### Clojure map vs pmap {#clojure-map-vs-pmap}

mapは [遅延シーケンス](#clojure-遅延評価-遅延シーケンス--laziness-evaluation-laziness-sequence) を構築し, [dorun/doall]({{< relref "20220116081924.md#dorun-doall" >}}) などで強制評価してはじめてリストが順次実行される.
...
[🔖map]({{< relref "20220305080412.md#map--高階関数" >}})
...
tag: [🏷filter]({{< relref "20220305080412.md#filter--高階関数" >}})
