...
## Collections一般 {#collections一般}


### リストにvalueを含むかという判定 {#リストにvalueを含むかという判定}
...
### もしnilでなければdo something, nilならばそのまま {#もしnilでなければdo-something-nilならばそのまま}
...
ref. [cond-> 条件つきスレッドマクロ]({{< relref "20220116162321.md#cond-条件つきスレッドマクロ" >}})
...
### nilならCollectionから取り除く {#nilならcollectionから取り除く}

(remove nil? coll) でいける.


## map->map: Mapの単体変換まとめ {#map-map-mapの単体変換まとめ}
...
### rename-keys: Mapのキーの名前変更 {#rename-keys-mapのキーの名前変更}
...
### 条件付きMap操作: assoc-if/update-if 追加する値がnilでなければ操作 {#条件付きmap操作-assoc-if-update-if-追加する値がnilでなければ操作}
...
### 条件付きMap操作: Mapにvalueが存在しないならば追加 {#条件付きmap操作-mapにvalueが存在しないならば追加}
...
### 複数のシーケンスのそれぞれの要素からmapのシーケンスを作成 {#複数のシーケンスのそれぞれの要素からmapのシーケンスを作成}
...
### ネストしたリスト(可変長引数)に関数を適用したい {#ネストしたリスト--可変長引数--に関数を適用したい}

関数の [可変長引数関数]({{< relref "20220116094551.md#clojure-可変長引数関数-variadic-functions" >}}) 対する関数適用で使えるテクニック.
...
greetingとwho(可変)に関数を適用するには [apply]({{< relref "20220116094551.md#clojure-apply-関数適用" >}}) をつかう.
...
### リストを一定の個数ごとにまとめたい(list of list aka. chunked list) {#リストを一定の個数ごとにまとめたい--list-of-list-aka-dot-chunked-list}
...
### (zipmap v1 v2) =>m : 複数のシーケンスからMap生成 {#zipmap-v1-v2--m-複数のシーケンスからmap生成}
...
## 手続き処理の中で処理をsleep {#手続き処理の中で処理をsleep}
